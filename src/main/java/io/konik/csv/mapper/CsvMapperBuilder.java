...
public class CsvMapperBuilder {
  ...
  public DozerBeanMapper buildBeanMapper(final Class<?> destinationObjectClass) {
    ...
    beanMapper.addMapping(new BeanMappingBuilder() {
      ...
      protected void configure() {
        TypeMappingBuilder readerBuilder =
            mapping(CsvDozerBeanData.class, destinationObjectClass, TypeMappingOptions.oneWay(),
                TypeMappingOptions.wildcard(false), TypeMappingOptions.mapNull(false));

        TypeMappingBuilder writerBuilder =
            mapping(destinationObjectClass, type(CsvDozerBeanData.class).mapNull(true),
        ...
        for (int i = 0; i < columns.size(); i++) {
          ...
          if (column == null) {
            throw new NullPointerException(
                String.format("fieldMapping at index %d should not be null", i));
          }
  ...
  public static CsvMapperBuilder withHeadersFromCsvFile(final File csvFile,
    ...
    try {
      ...
      List<Column> columns =
          Lists.transform(Arrays.asList(headers), new Function<String, Column>() {
            @Nullable
            @Override
  ...
  private static boolean isDelimiter(String delimiter, String lineOne, String lineTwo) {
    return lineOne.split(delimiter).length == lineTwo.split(delimiter).length
        && lineOne.contains(delimiter);
  }

  public CsvDozerBeanReader getBeanReader(File csvFile, Class<?> beanType) {
    try {
      CsvDozerBeanReader reader =
          new CsvDozerBeanReader(new InputStreamReader(new FileInputStream(csvFile), "UTF-8"),
              csvPreference, buildBeanMapper(beanType));
  ...
  static class Column {
    ...
    public Column(Builder builder) {
      ...
      this.mappingOptions =
          builder.mappingOptions.toArray(new FieldsMappingOption[builder.mappingOptions.size()]);
    ...
    public String toString() {
      return "Column{" + "name='" + name + '\'' + ", type=" + type + ", processor=" + processor
          + ", mappingOptions=" + Arrays.toString(mappingOptions) + ", fieldDefinition="
          + fieldDefinition + '}';
    }
