...
import io.konik.csv.mapper.CsvMapperBuilder.Column.Builder;
...
final class CsvMapperHeaderColumnsConfigurer implements ColumnsConfigurer {
  ...
  private static final ConcurrentMap<Pattern, Function<String, Column.Builder>> config =
      new ConcurrentHashMap<Pattern, Function<String, Column.Builder>>();
  ...
  private static Function<String, Column.Builder> typeColumn(final Class<?> type,
    ...
    return new Function<String, Column.Builder>() {
      @Nullable
      @Override
      public Column.Builder apply(@NotNull String input) {
  ...
  public Column getColumnDefinitionForHeader(String header) {
    for (Entry<Pattern, Function<String, Builder>> pattern : config.entrySet()) {
      Matcher matcher = pattern.getKey().matcher(header);

      while (matcher.find()) {
        String value = matcher.group();

        Column.Builder builder = config.get(pattern.getKey()).apply(value);
