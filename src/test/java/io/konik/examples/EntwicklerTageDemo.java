...
public class EntwicklerTageDemo {
  ...
  private Document createOrder() {
    ...
    order.setHeader(new Header().setInvoiceNumber("20151106-42").setCode(_220).setIssued(today)
        .setName("Bestellung"));
    ...
    trade.setSettlement(new Settlement().setPaymentReference("PER-42").setCurrency(EUR)
        ...
        .setMonetarySummation(new MonetarySummation().setLineTotal(new Amount(498, EUR))
            .setChargeTotal(new Amount(0, EUR)).setAllowanceTotal(new Amount(0, EUR))
            .setTotalPrepaid(new Amount(0, EUR)).setTaxBasisTotal(new Amount(498, EUR))
            .setTaxTotal(new Amount("74.70", EUR)).setDuePayable(new Amount("572.70", EUR))
  ...
  public void creatOrder() throws IOException {
    ...
    zfHandler.appendInvoice(order, inputPdf,
        new FileOutputStream("target/acme_invoice-42_ZUGFeRD.pdf"));
  }
  ...
  public void validateIncomingInvoiceForError() {
    ...
    PdfHandler zfHandler = new PdfHandler();
    Invoice invoice =
        zfHandler.extractInvoice(getClass().getResourceAsStream("/Jochen_invoice.pdf"));
