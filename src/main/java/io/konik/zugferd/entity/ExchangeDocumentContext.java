...
public class ExchangeDocumentContext {
    ...
    */
   public ExchangeDocumentContext(InvoiceProfile profile) {
      addProfile(new DocumentContextParameter(profile.fullName));
   }
   ...
   public InvoiceProfile getInvoiceProfile() {
      String profileFullName = getProfile().iterator().next().getId().getValue();
      return InvoiceProfile.getProfile(profileFullName);
   }
