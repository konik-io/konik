...
public class RowToInvoiceConverterTest {
	...
	public void shouldConvertNote() {
		...
		assertThat(invoice.getHeader().getNotes().get(0).getContents()).hasSize(1).contains("This is simple note in two separate lines", Index.atIndex(0));
	}
	...
	public void shouldConvertBuyerTaxInformation() {
		...
		assertThat(invoice.getTrade().getAgreement().getBuyer().getTaxRegistrations().get(0)).is(equalToRowTradePartyTaxInfo(row.getRecipient().getTaxes().get(0)));

		assertThat(invoice.getTrade().getAgreement().getBuyer().getTaxRegistrations().get(1)).is(equalToRowTradePartyTaxInfo(row.getRecipient().getTaxes().get(1)));
	}
	...
	public void shouldConvertSellerTaxInformation() {
		...
		assertThat(invoice.getTrade().getAgreement().getSeller().getTaxRegistrations().get(0)).is(equalToRowTradePartyTaxInfo(row.getIssuer().getTaxes().get(0)));
	}
	...
	public void shouldConvertItems() {
		...
		for (int i = 0; i < row.getItems().size(); i++) {
			assertThat(invoice.getTrade().getItems().get(i)).is(equalToRowItem(row.getItems().get(i), row.getHeader().getCurrency()));
		}
	}
