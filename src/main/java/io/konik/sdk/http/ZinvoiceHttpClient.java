...
public class ZinvoiceHttpClient {
  ...
  public static final HttpRequestFactory DEFAULT_HTTP_REQUEST_FACTORY =
  ...
  public InputStream download(String endpoint) {
    try {
      HttpRequest request = httpRequestFactory.buildGetRequest(createEndpoint(endpoint));
      request.setHeaders(
          new HttpHeaders().set("API-KEY", apiConfig.getApiKey()).setAccept("application/json"));
  ...
  public <T> T upload(String endpoint, Map<String, InputStream> files, Class<T> responseTypeClass) {
    try {
      ...
      for (Entry<String, InputStream> key : files.entrySet()) {
        InputStream file = new ByteArrayInputStream(ByteStreams.toByteArray(key.getValue()));
        MediaType mediaType = detector.detect(file, new Metadata());
        MultipartContent.Part part =
            new MultipartContent.Part(new InputStreamContent(mediaType.toString(), file));
        part.setHeaders(new HttpHeaders().set("Content-Disposition",
            String.format("form-data; name=\"%s\"; filename=\"%s\"", key.getKey(), key.getKey())));
